...
differential_phosphorylated_peptides <- krsa_group_diff(data_modeled$scaled, c("CONTROL", "ZORI + NOISE"), pep_passed_qc, byChip = T)

# save LFC table
write_csv(differential_phosphorylated_peptides, "results/dpp_CONTROL_ZORI + NOISE_STK.csv")
...
krsa_heatmap(data_modeled$normalized, significant_peptides$meanLFC$`0.2`, groups = c("CONTROL", "ZORI + NOISE"), scale = "row", width = 6, height = 8)
...
krsa_violin_plot(data_modeled$scaled, significant_peptides$meanLFC$`0.2`, "Barcode", groups = c("CONTROL", "ZORI + NOISE"))
...
mutiple_krsa_outputs <-
  future_map(sigPeps_total, krsa, .options = furrr_options(seed = T))
saveRDS(mutiple_krsa_outputs, file = "multiple_krsa_output_CONTROL_ZORI + NOISE_STK.RDS")
...
AvgZTable2 %>%
  arrange(desc(abs(AvgZ))) %>%
  slice_head(n = 10) %>%
  pull(Kinase) -> kinases_hits
...
#| fig-height: 6
#| fig-width: 6
#| fig-cap: "Kinase Activity summary for each kinase family based on peptide phsophorylation"
...
#| fig-height: 6
#| fig-width: 6
#| fig-cap: "Percentge of peptides each kinase family phosphorylates"
